...
| Hilfsfunktionen       | `array-utils.ts`, `match-utils.ts`, `color-filter-match.ts`, `color-combo-names.ts`, `card-effect-filters.ts`, `commander-archetype-filters.ts`, `rank-sort.ts`, `bracket.ts`, `combo-finder.ts`, `deck-metrics.ts`, `goldfish-sim.ts`, `mana.ts`                  |
...
- **Auswertungen:** `docs/` — Ergebnisberichte samt Messwerten (`deck-metriken-2026-09.md` + `.csv`).
...
- Es gibt nur **12 Spec-Dateien** (`scryfall.service`, `public-deck.service`, `color-filter-match`, `color-combo-names`, `app-recovery`, `bracket`, `combo-finder`, `deck-metrics`, `goldfish-sim`, `mana`, `ui/radar-chart/radar-geometry`, `i18n/i18n-keys`). Die Tests sind **kein Sicherheitsnetz** — grüne Tests sagen fast nichts.
