...
- **Die Sieg-Definition steht genau einmal:** `SIEG_MUSTER` in `src/app/goldfish-sim.ts`, in der Datenbank als `spellbook_winning_combo_muster()` (`sql/winning-combo-muster-2026-09-16.sql` — die Migration baut die materialisierte Ansicht mit genau diesem Ausdruck neu und legt `winning_combos_in_deck` danach unverändert wieder an). Sie stand früher doppelt (TypeScript und SQL) und lief auseinander — die eine Fassung zählte „You lose the game“ als Sieg, die andere verpasste „All opponents lose the game“. `scripts/simulate-deck-pool.js` vergleicht beide vor jedem Lauf und bricht bei Abweichung ab. Wer den Ausdruck ändert: **kein `\b`** — in Postgres ist das ein Rückschritt-Zeichen, keine Wortgrenze.
