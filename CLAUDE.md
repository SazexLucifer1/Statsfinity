...
- **Goldfish-Simulation (GitHub Action, NICHT nächtlich):** `scripts/simulate-deck-pool.js` (Workflow `deck-sim.yml`, füllt `deck_sim_results`, Tabelle und Auswertungs-Ansichten in `sql/deck-sim-results-2026-09-16.sql`). Spielt jedes Deck aus dem Archidekt-Vorrat ein paar hundert Mal ohne Gegner aus und misst, in welchem Zug es gewinnen könnte — plus gezählte Kennzahlen (Rampe, Kartenziehen, Interaktion, Tutoren, Manakosten). Zweck: die offiziellen Bracket-Kriterien zählen Karten und messen kein Tempo; dieser Lauf liefert die Gegenprobe an Decks, deren Stufe jemand anders festgelegt hat. Die fertige Gegenüberstellung steht in der Ansicht `deck_sim_by_bracket` — eine Spalte, deren Werte von Stufe 2 zu Stufe 4 nicht monoton laufen, trennt die Stufen nicht. **Die Spiellogik steht NICHT im Skript**, sondern in `src/app/goldfish-sim.ts` und `src/app/sim-card-profile.ts`, weil dieselbe Rechnung später im Browser für ein einzelnes Deck laufen soll; das Skript bündelt die beiden Module zur Laufzeit mit esbuild. Braucht `SUPABASE_SERVICE_ROLE_KEY`, `--dry-run` rechnet ohne zu schreiben. Der Simulator braucht `power`/`toughness` in `scryfall_cards` (`sql/scryfall-sim-fields-2026-09-16.sql`) — nach dieser Migration den Scryfall-Abgleich **mit `--force`** anstoßen, sonst lädt er die Bulk-Datei gar nicht erst und die neuen Spalten bleiben leer.
