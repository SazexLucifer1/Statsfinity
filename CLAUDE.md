...
- **Supabase-Migrationen:** `sql/` (datierte Skripte). **Diese Dateien laufen nicht automatisch** — sie müssen von Hand im Supabase-SQL-Editor ausgeführt werden, ein Merge allein ändert an der Datenbank nichts. Wer einem Fehler nachgeht, der nach „die App speichert nicht“ aussieht, prüft deshalb zuerst die Browser-Konsole: Rechte-Fehler kommen als HTTP 500 mit Postgres-Codes wie `42P17` an, nicht als Code-Fehler (siehe `sql/fix-tournament-rls-recursion-2026-09-03.sql`). **Wenn die Datenbank am 500-MB-Limit des Free-Plans steht:** `sql/datenbankgroesse-pruefen-2026-09-20.sql` zeigt Tabellengrößen, Aufblähung und Zuwachs je Tabelle — inklusive einer Messtabelle, die beim zweiten Lauf den Unterschied zwischen „ist groß“ und „wächst“ sichtbar macht. Zwei Posten wachsen dort von selbst: die nächtlichen Abgleiche schreiben rund 540.000 Zeilen pro Nacht neu (jedes `update` lässt eine tote Zeile zurück, und `autovacuum` gibt den Platz nur innerhalb der Datei frei, nie an die Platte), und `deck_sim_results` behält jede Simulator-Fassung, weil `sim_version` im Primärschlüssel steht.
