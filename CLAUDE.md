...
- **Deck-Löschen ist ein „Grabstein", kein `delete`:** `DeckService.deleteDeck()` entfernt ein Deck nur dann wirklich, wenn **keine einzige Partie** daran hängt. Sonst werden `deck_cards` und `deck_change_log` gelöscht (das ist der Teil, der Platz kostet), die `decks`-Zeile bleibt mit `deleted_at` plus gerettetem Commander stehen (`sql/deck-grabstein-loeschen-2026-09-21.sql`). Grund: `match_players.deck_id` ist `on delete set null` — ein hartes Löschen riss Deckname, Besitzer und Farbidentität aus jeder Partie und verschob die Spiele aus der Deck- in die Commander-Wertung. **Wer eine neue Deck-Abfrage schreibt, entscheidet bewusst:** Listen/Auswahl/öffentliche Suche filtern über `DeckService.nurLebende(query)`, Statistik-Abfragen (`getColorIdentities`, Namens-Joins in `mtg.service.ts`, die globalen SQL-Funktionen) dürfen **nicht** filtern — sonst ist der Verlust wieder da. Fehlt die Migration noch, schaltet sich der Filter beim ersten `42703` selbst ab (`grabsteinSpalteVerfuegbar`, gleiche Mechanik wie bei den Bracket-Spalten), und Decks mit Partien lassen sich solange nicht löschen, statt sie ungeschützt zu verlieren.
