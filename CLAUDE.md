...
- **Einmaliger Deck-Korpus-Import (GitHub Actions, kein Zeitplan):** `scripts/import-deck-corpus.js` sammelt legale Commander-Decks aus drei Quellen (MTGJSON-Precons, die eigenen öffentlichen Decks, Archidekt) als gepackte JSONL-Dateien im privaten Supabase-Storage-Bucket `deck-corpus`; `scripts/aggregate-deck-corpus.js` rechnet daraus die Empfehlungszahlen in `deck_corpus_commanders`/`deck_corpus_commander_cards`/`deck_corpus_card_baseline`. Tabellen und Begründung in `sql/deck-corpus-2026-09-08.sql`, Workflow `deck-corpus-import.yml`, gleiches Repo-Secret `SUPABASE_SERVICE_ROLE_KEY`. Das ist die Grundlage für ein **eigenes** Empfehlungssystem als Ersatz für `edhrec.service.ts` — die App liest davon noch nichts. **Die Archidekt-Quelle steht still, bis von dort eine Erlaubnis vorliegt** (siehe `ARCHIDEKT-ANFRAGE.md`); das Skript verlangt dafür `--erlaubnis-liegt-vor`. Der Torwächter im Import-Skript lässt nur genau 100 Karten große, farbidentitäts- und singleton-konforme Commander-Decks durch und ist ohne Netz prüfbar: `node scripts/import-deck-corpus.js --selftest`.
