...
| Hilfsfunktionen       | `array-utils.ts`, `match-utils.ts`, `color-filter-match.ts`, `color-combo-names.ts`, `card-effect-filters.ts`, `commander-archetype-filters.ts`, `rank-sort.ts`, `bracket.ts`, `combo-finder.ts`, `deck-metrics.ts`, `goldfish-sim.ts`, `mana-symbols.ts`, `mana-symbols.generated.ts`                  |
...
- **Forschungslauf (kein Nachtlauf, kein Workflow):** `scripts/analyze-deck-corpus.js` misst die Kennzahlen aus `deck-metrics.ts`/`goldfish-sim.ts` an mehreren hundert cEDH-Decks (EDHREC), denselben Commandern ohne cEDH-Filter und den Precons, und schreibt Ergebnis und Messwerte nach `docs/`. Wird von Hand angestoßen, wenn die Zahlen neu erhoben werden sollen — er liest Supabase nur, der Anon-Key reicht.
- **Auswertungen:** `docs/` — Ergebnisberichte samt Messwerten (`deck-metriken-2026-09.md` + `.csv`) sowie `docs/mtg-regeln.md`: die wörtlich zitierten Comprehensive-Rules-Absätze, auf die sich `goldfish-sim.ts` und `mana-symbols.ts` mit Regelnummer berufen. Wer dort eine Regel ändert, ändert sie hier mit.
- **Manasymbol-Tabelle:** `scripts/generate-mana-symbols.js` holt die 75 Manasymbole von Scryfalls `/symbology` und schreibt sie nach `src/app/mana-symbols.generated.ts`. Anders als `version.ts` ist diese erzeugte Datei **eingecheckt** — sie ändert sich nur, wenn Wizards ein neues Symbol druckt.
...
- Es gibt nur **12 Spec-Dateien** (`scryfall.service`, `public-deck.service`, `color-filter-match`, `color-combo-names`, `app-recovery`, `bracket`, `combo-finder`, `deck-metrics`, `goldfish-sim`, `mana-symbols`, `ui/radar-chart/radar-geometry`, `i18n/i18n-keys`). Die Tests sind **kein Sicherheitsnetz** — grüne Tests sagen fast nichts.
